// Piratwhist Online Multiplayer (v1.1.0)
...
// --- v1.1.0: dynamic round-table board (2–8 players) ---
...
function pcLayoutTunerActive(){
  // v1.1.0: Layout-tuner panelet må kun være synligt for spillernavn "LaBA".
...
function positionPlayBoard(n){
  ...
  if (isMobile){
    // v1.1.0 Dev + layout: SceneShift for mobile to utilize top space and
...
// v1.1.0 PC HUD sync + button wiring
function syncPcHud(){
...
// v1.1.0 no-fly zone: avoid overlap between hand area and the bottom-left opponent seat on PC
