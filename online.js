// Piratwhist Online Multiplayer (v1.2.7)
...
// --- v1.2.7: dynamic round-table board (2–8 players) ---
...
function pcLayoutTunerActive(){
  // v1.2.7: Layout-tuner panelet må kun være synligt for spillernavn "LaBA".
...
function getPwAiBaseUrl(){
  try{
    // Prefer global AI URL (from /ai-url via global_ai.js) so all devices work automatically
    if (typeof window.PW_getAiBaseUrl === "function") {
      const g = window.PW_getAiBaseUrl();
      if (g) return normalizePwAiBaseUrl(g);
    }
...
function initPwAiHelp(){
  ...
  const closeAll = () => {
    ...
    if (audioEl) { try{ audioEl.pause(); }catch(e){} audioEl.removeAttribute("src"); }

  };


  async function pwAiCheckHealth(){
    const baseUrl = normalizePwAiBaseUrl(urlInput?.value || getPwAiBaseUrl());
        if (!baseUrl){
  ...
  const open = async () => {
    ...
    // On new devices (mobile/other PC) global URL may arrive async – refresh once if empty
    if (urlInput && !urlInput.value && typeof window.PW_refreshAiBaseUrl === "function") {
      try{
        const u = await window.PW_refreshAiBaseUrl();
  ...
  btnSpeak?.addEventListener("click", async () => {
    ...
    try{
      ...
      if (status) status.textContent = "Kunne ikke afspille lyd (tjek at /speak virker).";
    }finally{
      btnSpeak.disabled = !lastAnswerText;
  ...
  ask?.addEventListener("click", async () => {
    ...
    try{
      ...
      lastAnswerText = a;
      if (btnSpeak) btnSpeak.disabled = !lastAnswerText;
...
function positionPlayBoard(n){
  ...
  if (isMobile){
    // v1.2.7 Dev + layout: SceneShift for mobile to utilize top space and
...
// v1.2.7 PC HUD sync + button wiring
function syncPcHud(){
...
// v1.2.7 no-fly zone: avoid overlap between hand area and the bottom-left opponent seat on PC
